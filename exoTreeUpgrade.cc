...
void exoTreeUpgrade () {
	cout << "-----------------------------------------" << endl << "Wellcome to EXO-tree-upgrade!" << endl << "-----------------------------------------" << endl; 

	gROOT->SetBatch(1); 
	TCanvas c1 ("c1", "c1", 500, 500); 

	//chooses the tree to work with
	TFile *_file0 = TFile::Open ("/afs/cern.ch/user/r/rdangovs/private/EXO80/CMSSW_8_0_8_patch1/src/flashgg/output.root");	
	//cout << "Hello!" << endl;
...
vector<Plot> getPlotDetails(){
    ...
    info.var = "cosThetaStar";
    info.numBins = 40;
    info.minX = -1;
    info.maxX = 1;
    info.category = 0;
    info.title = "#cos (#theta^{#star})";
    info.xLabel = "#cos (#theta^{#star})";
    info.yLabel = "dN/d#cos (#theta^{#star})";
    info.isXLog = false;
    info.isYLog = false;
    plotInfo.push_back(info);
